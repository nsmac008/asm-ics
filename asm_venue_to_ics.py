...
v7 — writes output to public/asm_calendar.ics so GitHub Pages reliably serves it.
...
def extract_title_from_html(html: str) -> str | None:
    """Best-effort title extraction from event HTML."""
...
def find_dates_and_title_from_html(html: str):
    ...
    # 3) Visible text like "Oct 24, 2025 | 7:00 PM"
...
def collect_event_links_from_location(html: str, base_url: str) -> list[str]:
    ...
    # Look for obvious event card links
    ...
    # Some sites embed a JSON array of events with "url" fields
...
def main():
    ...
    # Gather event detail URLs from the venue page
    event_urls = collect_event_links_from_location(html, start_url)

    events: list[Event] = []

    # If no links found, try to parse dates directly from the venue page
    if not event_urls:
    ...
    # Visit each event page for definitive times and *titles*
    for u in event_urls:
        try:
            eh = fetch(u)
        except Exception:
    ...
    if not events:
        # Emit a visible test VEVENT so subscribers can verify the feed is live
        s = now_ny() + timedelta(days=1, hours=9)
        events.append(
            Event(
                "ASM Venue Feed Connected — awaiting events",
                s,
                s + timedelta(hours=1),
                url=start_url,
            )
    ...
    # Sort & write → public/asm_calendar.ics
